...
accept-exact-dirs
    これはファイル名の補完で使用されます。accept-exactと異なり、ブール値です。
    ファイル名補完は、たとえコンポーネントが実在するディレクトリにマッチして\
    いても、コンポーネントの補完があるかどうかを確認するために、すべての\
    コンポーネントを検査します。
    例えば/usr/bin/の後の補完で、関数は/usrに対して補完候補を検査します。

    このスタイルがtrueの場合、既存のディレクトリに一致するパスの接頭辞は、\
    それ以上補完しようとせずに受け入れられます。したがって、上記の例では\
    /usr/bin/はすぐに受け入れられ、そのディレクトリの中を補完しようとする\
    でしょう。

    もしこの振る舞いを全体的に禁止したいなら、path-completionスタイルを\
    falseに設定してください。

add-space
    このスタイルは_expandコンプリータによって使用されます。
    true（デフォルト）の場合、展開後のワード全体の後ろに空白が挿入され、\
    ディレクトリ名の場合はスラッシュが挿入されます。値が'file'の場合、\
    コンプリータは既存のファイル名にのみ空白を追加します。
    ブール値trueまたは値'file'は、'subst'と組み合わせることができ、その\
    場合には、'$(...)'や'${...}'の形式の置換の展開から生成されたワードに\
    対しては空白を追加しません。

    _prefixコンプリータは、スペースを接尾辞の前に挿入する必要があるかどうか\
    を決定するために、単純なブール値としてこのスタイルを使用します。

ambiguous
    これはファイル名パスの最後でないコンポーネント、つまり末尾にスラッシュが\
    付いたものを補完する際に適用されます。設定されていれば、メニュー補完を\
    使っていたとしても、カーソルは最初の不確定コンポーネントの後ろに残され\
    ます。このスタイルはpathsタグに対して常に試行されます。

assign-list
    代入として扱われている等号の後ろで補完すると、補完システムは通常、\
    ファイル名を1つだけ補完します。いくつかのケースで、この値はPATHや類似の\
    パラメータのように、コロンで区切られたファイル名のリストであることが\
    あります。このスタイルは、そのようなパラメータの名前と一致するパターンの\
    リストを設定できます。

    デフォルト（の動作）は、コマンドライン上のワードにコロンが含まれている\
    場合にリストを補完します。

auto-description
    設定されている場合、このスタイルの値は、補完関数で記述されていない\
    オプションの説明として使われ、必ず1つ引数を持ちます。値の中のシーケンス\
    '%d'は、この引数の説明に置き換えられます。
    個人の好みに応じて、このスタイルを'specify: %d'などのように設定すると\
    便利かも知れません。これはいくつかのコマンドに対しては動作しない場合が\
    あることに注意してください。

avoid-completer
    これは、文字列がすべての一致候補で構成される場合に、現在生成されている\
    リストに追加すべきかを決定するために_all_matchesコンプリータが使用します。
    その値はコンプリータ名のリストです。
    これらのいずれかが、この補完でマッチを生成したコンプリータの名前の場合、\
    文字列は追加されません。

    このスタイルのデフォルト値は '_expand _old_list _corrent _approximate'\
    です。すなわち、おおよそ必要とされない一致候補を含む文字列に対する\
    コンプリータを含みます。

cache-path
    このスタイルはダンプされた補完データを含む、あらゆるキャッシュファイルが\
    保存されるパスを定義します。$ZDOTDIR が定義されていない場合、デフォルトは\
    '$ZDOTDIR/.zcompcache'または'$HOME/.compcache'です。use-cacheスタイルが\
    設定されない限り、補完キャッシュは使用されません。

cache-policy
    このスタイルはキャッシュの再構築が必要かどうかを決定するために使われる\
    関数を定義します。後述の_cache_invalid関数のセクションを参照してください。

call-command
    このスタイルはmakeやantのような、マッチを生成するコマンドを直接呼び出す\
    と遅かったり、makeの場合のように潜在的にmakefile内のアクションが実行\
    される可能性があるなどの問題があるコマンドを呼び出す関数で使われます。
    'true'に設定されている場合、コマンドはマッチを生成するために呼び出され\
    ます。このスタイルのデフォルトは'false'です。

command
    多くの場所で、補完関数は補完のリストを生成するための外部コマンドを\
    呼び出す必要があります。このスタイルはいくつかのそのような場合に、\
    呼びだされるコマンドを上書きするために使えます。値の要素は、実行する\
    コマンドラインを形成するようにスペースで区切られます。
    値はハイフンで始めることができ、その場合、通常のコマンドが最後に\
    追加されます; これは例えば、外部コマンドと同じ名前のシェルコマンドを\
    呼び出すことを避け、適切なバージョンのコマンドが呼びだされたことを\
    確認するために、'builtin'や'command'を前に置く場合に最も便利です。

    例として、プロセスIDの補完関数は、補完するためのIDと（verboseスタイルが\
    'true'であれば）表示するためのプロセスのリストを生成するため、\
    processesタグにこのスタイルを使っています。コマンドによって生成される\
    リストは、psコマンドの出力のようになるはずです。最初の行は表示されません\
    が、以降の行のプロセスIDの位置を見つけるために、文字列'PID'（または\
    'pid'）が検索されます。その行に'PID'を含んでいない場合、その他の各行の\
    最初の番号が補完するプロセスIDと見なされます。

    補完関数は一般的に、補完リストを生成しようとするたびに指定された\
    コマンドを呼び出す必要があることに注意してください。したがって、\
    実行に時間がかからないコマンドのみを指定し、特に終了しなくなる可能性が\
    あるものは何であれ避けるように注意するべきです。

command-path
    補完のためのコマンドを検索するディレクトリのリストです。このスタイルの\
    デフォルトは特別なパラメータパスの値です。

commands
    これは（/etc/init.d またはそこからそう遠くないディレクトリに存在する）\
    システム初期化スクリプトのサブコマンドを補完する関数によって使われます。
    その値は、補完関数が補完候補を自動的に見つけられない場合に、それらの\
    コマンドを補完するためのデフォルトの値を与えます。
    このスタイルのデフォルトは、2つの文字列'start'と'stop'です。

complete
    これはバインド可能なコマンドとして呼び出された際に、_expand_alias関数\
    によって使われます。'true'に設定され、コマンドライン上のワードが\
    エイリアス名でない場合、一致するエイリアス名が補完されます。

complete-options
    これはcd、chdirおよびpushdのコンプリータによって使われます。
    これらのコマンドでは - はディレクトリスタックエントリを案内するために\
    使われ、これらの補完はオプションを補完することよりもはるかに一般的です。
    したがって、このスタイルの値が'true'でない限り、最初の - の後であっても、\
    オプションは補完されません。もしtrueの場合、コマンドライン上で前に -- が\
    ある場合を除き、最初の - の後ろにオプションが補完されます。

completer
    このスタイルの値として与えられた文字列は、使用するコンプリータ関数の\
    名前を提供します。使用可能なコンプリータ関数は、後述する\
    'コントロール関数'のセクションで説明されています。

    各文字列は、コンプリータ関数の名前もしくは'function:name'の形式の文字列\
    です。1つ目のケースでは、コンテキストのcompleterフィールドは、先頭の\
    アンダースコアがなく、他のアンダースコアはすべてハイフンに置き換えられた\
    コンプリータ名が含まれます。2つ目のケースでは、functionは呼び出される\
    コンプリータの名前ですが、コンテキストのcompleterフィールドにはユーザ\
    定義の名前が含まれます。名前がハイフンで始まる場合、コンテキストの\
    文字列は、最初のケースのようにしてnameを付加したコンプリータ関数の名前\
    から作られます。例えば::

        zstyle ':completion:*' completer _complete _complete:-foo

    ここでは、補完はコンテキストのcompleterフィールドで、一度は'complete'を\
    使って、一度は'complete-foo'を使って _completeコンプリータを2回呼び出し\
    ます。通常、同じコンプリータを複数回使うことは、'function:name'の形式で\
    使われる場合にのみ意味があります。なぜなら、そうでない場合、コンテキスト\
    名はコンプリータに対するすべての呼び出しで同じになるからです; このルール\
    の例外となりえるのは_ignoredと_prefixコンプリータです。

    このスタイルのデフォルト値は '_complete _ignored'です: \
    最初はignored-patternsスタイルと$fignore配列を使い、それからマッチを\
    無視せずに補完が行われます。

condition
    このスタイルは、マッチの挿入が無条件に遅延する必要があるかどうかを\
    決定するため _listコンプリータ関数によって使用されます。デフォルト値は\
    'true'です。

delimiters
    このスタイルは区切り文字で区切られた引数をもつヒストリ修飾子やglob修飾子と\
    共に使用する区切り文字を追加する場合に使用します。これは追加すべき区切り\
    文字の配列です。補完システムが混乱してしまうため、非特殊文字の方が\
    望ましいでしょう。デフォルトのリストは、:、+、/、-、% です。リストは、\
    区切り文字の入力が強制されるように、空にすることもできます。

disabled
    これが'true'に設定されている場合、_expand_aliasコンプリータとバインド可能\
    コマンドは、無効化されているエイリアスも展開しようとします。
    デフォルトは'false'です。

domains
    補完するネットワークドメイン名のリストです。これが設定されていない場合、\
    ドメイン名は /etc/resolv.conf から取得されます。

environ
    environスタイルは'sudo'を補完する際に使われます。これは対象のコマンドに\
    対する補完が呼び出される前に、ローカル環境のエクスポートされる\
    'VAR=value'の代入の配列を設定します。
    zstyle ':completion:*:sudo::' environ \
      PATH="/sbin:/usr/sbin:$PATH" HOME="/root"

expand
    パス名のように、複数のパートから構成される文字列を補完するときに、この\
    スタイルが使われます。

    値のうちの一つが文字列'prefix'の場合、行に部分的に入力されたワードは、\
    末尾まで補完できないとしても可能な限り補完されます。

    値のうちの一つが文字列'suffix'の場合、最初に現れる曖昧なものの後にある\
    コンポーネントに対して一致する名前も追加されます。
    これは、結果の文字列が可能な限り最も長く一義的な文字列であることを意味\
    します。しかし、メニュー補完はすべての補完候補を順番に使えます。

fake
    このスタイルは任意の補完コンテキストを設定できます。そのコンテキストで\
    常に補完される追加の文字列を指定します。
    各文字列の形式は 'value:description' です; コロンとdescriptionは省略でき\
    ますが、値の中のリテラルのコロンはバックスラッシュでクオートしなければ\
    いけません。付与されたdescriptionは補完リストに値と共に表示されます。

    偽の文字列を指定する場合、十分に限定的なコンテキストを使うことが重要です。
    ファイルやパラメータを補完する場合には、fake-filesやfake-parametersが\
    追加の機能を提供していることを覚えておいてください。

fake-always
    これはignored-patternsスタイルが適用されないことを除いて、fakeスタイルと\
    同じように動作します。'\*'に無視パターンを設定することによって、マッチの\
    集合を完全にオーバーライドできます。

    以下は任意のデータでタグを補完する方法を示していますが、別々のタグの\
    ように表示するために振る舞います。この例では、標準コンプリータがcdの\
    引数を補完する際に、named-directoriesタグを２つに分割するために\
    tag-orderスタイルの機能を使用します。named-directories-normalタグは\
    通常どおりに振る舞いますが、named-directories-mineタグには、固定の\
    ディレクトリセットが含まれています。これは所定の補完にマッチグループ\
    'extra directories'を追加する効果を持ちます。 ::
...
fake-files
    このスタイルはファイルの補完とタグなしでの参照の際に使用されます。
    値は'dir:names...'の形式です。これはディレクトリdirの中で補完する際に、\
    たとえそのようなファイルが実在しなくてもマッチする可能性として（空白で\
    区切られた文字列の）名前を追加します。dirはパターンでも構いません; \
    パターン内の文字及びコロンはリテラルに扱われる必要がある場合には、\
    バックスラッシュでクオートする必要があります。

    これはトップレベルのパス名を一覧できない、またはglobパターンで生成できない\
    特殊なファイルシステムをサポートしているシステムで役に立つことがあります。
    また、読み取り権限を持っていないディレクトリにも使えます。

    パターンの形式は、特定のファイルシステム上のすべてのディレクトリに、\
    任意の'magic'エントリを追加するために使えます。

fake-parameters
    これはパラメータ名の補完関数によって使われます。値は、まだ設定されて\
    いないかも知れないが、補完される必要があるパラメータの名前です。
    それぞれの名前には、それぞれコロンと（'scalar'や'array'、'integer'の\
    ような）パラメータの種類を指定する文字列を続けることもできます。
    種類が与えられている場合、特定のコンテキストでその種類のパラメータが\
    必要な場合だけ、名前が補完されます。種類が指定されていない名前は、常に\
    保管されます。
